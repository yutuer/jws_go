...
func init() {
	...
	flag.StringVar(&_set, "set", "rand", "data/h1.txt or data/h100.txt (default rand).")
}
...
func main() {
	...
	fmt.Println(files)
	samples := make(map[string]redis.Args, 10)
	...
	for i := 0; i < _clients; i++ {
		...
		go func(id int) {
			...
			for w := range work {
				key := fmt.Sprintf(c_key_format, w.Idx)
				rparams := redis.Args{}.Add(key).AddFlat(w.Args)
				//fmt.Println(w.Idx,len(rparams))

				beforew := time.Now().UnixNano()
				//_, err2 := conn.Do("HMSET", rparams...)
				//if err2 != nil {
				//	panic(err2)
				//}
				err3 := conn.Send("HMSET", rparams...)
				if err3 != nil {
					panic(err3)
				}
	...
	case "data/h1.txt":
		for i := 0; i < _requests; i++ {
			work <- WorkLoad{i, samples["data/h1.txt"]}
		}
	case "data/h100.txt":
		for i := 0; i < _requests; i++ {
			work <- WorkLoad{i, samples["data/h100.txt"]}
		}
	default:
		for i := 0; i < _requests; i++ {
			//a := 0
			//fmt.Println(samples["data/h1.txt"])
			select {
			case work <- WorkLoad{i, samples["data/h1.txt"]}:
			case work <- WorkLoad{i, samples["data/h100.txt"]}:
			}
		}
	}
