...
func (rp *ResourcePool) get(ctx context.Context, wait bool) (resource Resource, err error) {
	...
	if wrapper.resource == nil {
		...
		if err != nil {
			select { // 因为外面使用的地方，还有可能往resources里放，所以这里做一下判断，放不进去就不放了
			case rp.resources <- resourceWrapper{}:
			default:
				logs.Critical("%v", errors.New("ResourcePool-get attempt to Put into a full ResourcePool"))
			}
		}
	}
...
func (rp *ResourcePool) Put(resource Resource) {
	...
	default:
		logs.Critical("%v", errors.New("ResourcePool-put attempt to Put into a full ResourcePool"))
	}
}
