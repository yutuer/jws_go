...
func (ma *PlayerMarketActivitys) OnLevel(acid string, levelType uint32, times int, now_t int64) bool {
	ma.UpdateMarketActivity(acid, now_t)
	typ := uint32(gamedata.ActLevelMode)
	...
	for _, pa := range _pa {
		...
		levelTypeReq := subCfg[1].GetFCValue2()
		if levelType != levelTypeReq {
			continue
		}
		isUpdate = true
		...
		logs.Debug("MarketActivitys OnCustoms %d %d %v", levelType, times, pa)
	}
	if isUpdate {
...
//TODO 更改任务类型
func (ma *PlayerMarketActivitys) UpdateOnLevel(acid string, now_t int64) bool {
	typ := uint32(gamedata.ActLevelMode)
	_pa := ma.getActByTyp(typ)
	if _pa == nil || len(_pa) <= 0 {
		return false
