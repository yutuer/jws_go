...
func RedisSaveDataToOtherKey(from, to string) bool {
	_db := GetDBConn()
	defer _db.Close()

	return RedisSaveDataToOther(_db, from, to)
}
...
var (
	ChangeNameErrByNameExist = errors.New("NameExist")
)

func ChangeNameToRedis(name, acid string, sid uint) error {
	_db := GetDBConn()
	defer _db.Close()

	if _db.IsNil() {
		return fmt.Errorf("ChangeNameToRedis db is nil")
	}

	ret, err := redis.Int(_db.Do("HSETNX", TableChangeName(sid), name, acid))
	if err != nil {
		return fmt.Errorf("ChangeNameToRedis HSETNX err %v", err)
	}
	if ret <= 0 {
		return ChangeNameErrByNameExist
	}
	return nil
}

const Name_DB_Counter_Name = "name_db"

// bool true = 成功
func RenameToRedis(oldName, newName, acid string, sid uint) (bool, error) {
	_db := GetDBConn()
	defer _db.Close()
	nameTable := TableChangeName(sid)
	lua_src := fmt.Sprintf(`
		local newName = KEYS[1]
		local oldName = KEYS[2]

		if redis.call("HSETNX", "%s", newName, "%s") == 1 then
		    redis.call("HDEL", "%s", oldName)
		    return "OK"
		end

		return ""
	`, nameTable, acid, nameTable)

	rc := _db.RawConn()
	lua_script := redis.NewScript(2, lua_src)
	lua_script.Load(rc)

	res, err := redis.String(lua_script.Do(rc, newName, oldName))

	return res == "OK", err
}

var (
	mutex203Rename sync.Mutex
)

// 应对203大区自带redis不支持lua命令，因此加锁串行执行，改名不是频繁操作（203迁移后干掉）
func RenameFor203(oldName, newName, acid string, sid uint) (bool, error) {
	_db := GetDBConn()
	defer _db.Close()

	mutex203Rename.Lock()
	defer mutex203Rename.Unlock()

	res, err := redis.Int(_db.Do("HSETNX", TableChangeName(sid), newName, acid))
	if err != nil {
		return false, err
	}
	if res == 0 {
	...
	if _, err := _db.Do("HDEL", TableChangeName(sid), oldName); err != nil {
		logs.Error("RenameFor203 HDEL err %s", err.Error())
	}
	return true, nil
}
