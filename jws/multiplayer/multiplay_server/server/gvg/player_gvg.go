...
func NewPlayer(s *link.Session) *GVGPlayer {
	r := &GVGPlayer{
		session: s,
	}
...
//func (p *GVGPlayer) SendMsg(msg []byte) error {
//	logs.Trace("SendMsg To %s : %v", p.AcID, msg)
//	return p.session.Send(msg)
//}

func (p *GVGPlayer) GetAcID() string {
	return p.AcID
}

func (p *GVGPlayer) GetSession() *link.Session {
	return p.session
}

func (p *GVGPlayer) Start() {
	defer func() {
		if err := recover(); err != nil {
			logs.Error("GVGPlayer Panic, session Err %v", err)
			debug.PrintStack()
		}
	}()
